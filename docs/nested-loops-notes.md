...
- `(` = `\` reflect if `grid[GP] != 0`. Distinguishes first entry (GP=0, pass
  through E) from re-entry (GP>0, reflect S->E).
- `P` = `grid[GP]++`. Leaves a breadcrumb for reversibility.
- `%` = `/` reflect if `grid[CL] != 0`. The loop condition. Reflects E->N into
  the body when true; passes through E to exit when false.
- `]` = GP East. On the exit path only. Advances GP to a fresh column for the
  next loop construct.
- `/` and `\` on the body row are unconditional mirrors that form the loop-back
  rectangle.

Flow:
1. **First entry** (IP going E): `(` passes (GP=0) -> `P` (GP=1) -> `%` reflects
   E->N (CL!=0) -> body going W -> `\` turns N -> `\` at top-right -> `/` at
   top-left turns S -> back to `(`.
2. **Re-entry** (IP going S): `(` reflects S->E (GP>0) -> `P` (GP++) -> `%`
   reflects E->N -> body -> ... repeat.
3. **Exit** (CL=0): `%` passes E -> `]` advances GP -> post-loop code.

## The Nested Loop Problem

If an inner loop also uses `( P %`, it needs `grid[GP]=0` on first entry. But
on the *second* outer iteration, the inner loop's GP cell still has breadcrumbs
from the first iteration. The inner `(` sees GP!=0 and reflects — treating it
as a re-entry when it should be a first entry.

This is the fundamental challenge. CL-conditional loops (using `%` or `&`
without GP) don't have this problem, but they can't distinguish first entry
from re-entry at all — the mirror geometry is unsolvable without `(`.

## The Solution: Monotonic GP Consumption

**GP advances forward and never retracts.** Every `(` except the very first is
preceded by `]`, which advances GP to a guaranteed-fresh cell with `grid[GP]=0`.

### Rule 1: `]` before every inner `(`

When nesting loops, emit `]` immediately before each inner `(` in the body.
This is on the body path, so it executes every outer iteration, advancing GP
...
loop's GP cell. This separates sequential loops from each other.
...
leave GP cells non-zero. When the IP returns to the outer `(`, GP points to
...
### Rule 4: GP cost is O(outer_iters * inner_loops)

Each outer iteration consumes one GP column per inner loop (each `]` advances
one column). For factorial with N=5 (two inner loops — multiply and cleanup),
that's ~10 GP cells. A 32-wide grid gives ~30 usable columns, enough for
moderate inputs.

## Reversibility: "Burning Zeroes"

The GP trail is a reversibility resource. `P` increments cells; the reverse
operation `Q` decrements them. `]` advances GP; `[` retracts it. When running
backward, the simulator reconstructs the exact forward path.

The key insight: **you must burn zeroes in the garbage bytes.** Fresh GP cells
...
H1, GP — and the GP trail is completely clean (all zeroes).
...
```
  ...
  Row 3:  GP trail (zeroes, GP starts here)
...
each `(`. The cleanup loop uses `& P - $` (a variant using `$` = `/` if GP!=0
and `&` = `\` if CL!=0).
