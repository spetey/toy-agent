...
Each gadget's H2 scan head sweeps the other gadget's code rows.
...
- **Spatial locality**: interleaving means each gadget's code is
  physically adjacent to the other's, minimizing H2 travel distance.
...
- GP row placement: each gadget needs its own GP scratch space. Interleave
  GP rows too, or place them at the boundary?
