...
class InputDecl:
    ...
    def __repr__(self):
        return f"InputDecl({self.bytes})"

class ReadVar:
    """read x — grid[H0] += grid[H1]"""
    def __init__(self, var):
        self.var = var
    def __repr__(self):
        return f"ReadVar({self.var})"

class Advance:
    """advance — move H1 east"""
    def __repr__(self):
        return "Advance()"
    ...
    def parse_stmt():
        ...
        if tok == 'var':
            ...
            while peek() is not None and peek().isdigit():
                bytes_list.append(int(advance()))
            return InputDecl(bytes_list)
        elif tok == 'read':
            advance()
            var = advance()
            return ReadVar(var)
...
class Compiler:
    ...
    def _move_h1_to(self, target_row, target_col, emit_fn):
        if self.h1 is None:
            raise RuntimeError(
                "Cannot move H1: position unknown (after streaming loop). "
                "Use G opcode to restore H1 before moving it.")
        cr, cc = self.h1
        while cr < target_row:
            emit_fn('s'); cr += 1
        while cr > target_row:
    ...
    def write_fb2d(filename, rows, cols, grid_flat, header):
        ...
        with open(filename, 'w') as f:
            f.write(f"# Compiled from ifb by ifbc-03.py v0.3\n")
...
class CompilerV2:
    ...
    def __init__(self):
        ...
        self.h0 = (DATA_ROW, 0)
        self.h1 = (DATA_ROW, 0)
        self.h1_stream = None  # stream pointer, set when input is declared
        self.cl = (DATA_ROW, 0)
    ...
    def compile(self, program):
        # Pass 1: collect vars, input data, and find max nesting depth
        for stmt in program.stmts:
            if isinstance(stmt, VarDecl):
                col = len(self.variables)
                self.variables[stmt.name] = col
                ...
                self.input_bytes = stmt.bytes

        if not self.variables:
            self.variables['_dummy'] = 0
            self.data_values[0] = 0

        # If input is declared, allocate a hidden _h1save variable for
        # saving/restoring H1 stream position around variable ops in loops.
        if self.input_bytes:
            h1save_col = len(self.variables)
            self.variables['_h1save'] = h1save_col
            self.data_values[h1save_col] = 0

        num_vars = len(self.variables)

        # Place input data on row 0 past variables (with one gap column)
        if self.input_bytes:
            self.input_start_col = num_vars + 1
            for i, b in enumerate(self.input_bytes):
                self.data_values[self.input_start_col + i] = b
            # Initialize H1 and stream pointer to first input byte
            self.h1 = (DATA_ROW, self.input_start_col)
            self.h1_stream = (DATA_ROW, self.input_start_col)
        ...
        # Determine grid size
        input_end = (self.input_start_col + len(self.input_bytes)
                     if self.input_bytes else 0)
        rows = max(self.ex_row + 1, 4)
        cols = max(self.max_col + 2, num_vars + 1, input_end + 1, 8)
        ...
        # H1 initial position: input start if declared, else (0,0)
        if self.input_start_col is not None:
            h1_flat = DATA_ROW * cols + self.input_start_col
        else:
            h1_flat = DATA_ROW * cols + 0
    ...
    def _compile_stmts(self, stmts, level, cursor):
        ...
        for s in stmts:
            if isinstance(s, (VarDecl, InputDecl)):
                continue
    ...
    def _compile_one(self, stmt, level, cursor):
        ...
        elif isinstance(stmt, ReadVar):
            # read x — grid[H0] += grid[H1], reading from stream position
            col = self._var_col(stmt.var)
            self._move_h0_to(DATA_ROW, col, emit_here)
            if self.h1_stream is None:
                raise RuntimeError("'read' used without 'input' declaration")
            if self.in_streaming_loop:
                # Inside streaming loop: H1 is already at stream position
                # (from loop entry or after G-restore on re-entry).
                # Don't move H1.
                pass
            else:
                # Outside loop: move H1 to stream position explicitly
                self._move_h1_to(*self.h1_stream, emit_here)
            emit_here('.')   # grid[H0] += grid[H1]
        elif isinstance(stmt, Advance):
            # advance — move H1 east by one cell in the input stream
            if self.h1_stream is None:
                raise RuntimeError("'advance' used without 'input' declaration")
            emit_here('e')   # H1 moves east
            # Update stream pointer (compile-time tracking)
            self.h1_stream = (self.h1_stream[0], self.h1_stream[1] + 1)
            if self.in_streaming_loop:
                # G-save: store H1 stream pos in _h1save, load old _h1save into H1.
                # After this, H1 = flat(0,0) (or whatever _h1save held = consistent (0,0)).
                h1save_col = self._var_col('_h1save')
                self._move_h0_to(DATA_ROW, h1save_col, emit_here)
                emit_here('G')   # swap(H1_register, grid[H0=_h1save])
                # Compiler: H1 is now at flat 0 = (DATA_ROW, 0) on every iteration
                # because _h1save is always restored to flat(0,0) by G-restore.
                self.h1 = (DATA_ROW, 0)
            else:
                self.h1 = (self.h1[0], self.h1[1] + 1)
        elif isinstance(stmt, OutputVar):
            # output x — write variable to EX trail, zeroing it
            # Identical to ZeroVar: ] Z
            col = self._var_col(stmt.var)
            self._move_h0_to(DATA_ROW, col, emit_here)
            emit_here(']')   # advance EX to fresh cell (value 0)
            emit_here('Z')   # swap(grid[H0], grid[EX]) — value goes to EX trail
        elif isinstance(stmt, While):
    ...
    @staticmethod
    def _body_has_advance(stmts):
        """Check if a list of statements contains any Advance node (recursively)."""
        for s in stmts:
            if isinstance(s, Advance):
                return True
            if isinstance(s, While):
                if CompilerV2._body_has_advance(s.body):
                    return True
        return False
    ...
    def _compile_while(self, stmt, level, cursor):
        ...
        STREAMING SUPPORT (v0.3):
          When the loop body contains `advance`, H1 is being used as a
          stream pointer that must NOT be reset by the corridor.
          - Before %: move H1 back to h1_stream (stream position)
          - Corridor: zero H1 ops (don't reset — H1 stays advanced)
          - After exit: self.h1 = None (position is runtime-dependent)
        ...
        Corridor on corridor_row(level):
          / at col_L, \\ at col_R.
          Between them (going W): H0 reset ops, (H1 reset ops if no advance).
        ...
        has_advance = self._body_has_advance(stmt.body)
        ...
        h0_entry = self.h0
        h1_entry = self.h1
        h1_stream_entry = self.h1_stream  # stream position at loop entry
        # CL entry is (DATA_ROW, var_col), guaranteed above

        # ── For streaming loops: position H1 at stream for first entry ──
        if has_advance and self.h1_stream is not None:
            self._move_h1_to(*self.h1_stream, emit_here)

        # ── ] to ensure EX is on a fresh zero cell ──
        # This is needed because prior code (output, zero) may have left
        # grid[EX] non-zero. Without this, ( would reflect on first entry.
        emit_here(']')
        ...
        prev_streaming = self.in_streaming_loop
        if has_advance:
            self.in_streaming_loop = True
        for i, s in enumerate(stmt.body):
            if isinstance(s, While):
                # Inner while — _compile_while emits its own ] before (
                # to ensure EX is on a fresh zero cell for first entry.
                cursor = self._compile_while(s, level + 1, cursor)
                # After inner loop exit: EX points to the inner loop's
                # P-written breadcrumb cell (non-zero). This is exactly
                # what the outer ( needs on re-entry (grid[EX]!=0).
            else:
                cursor = self._compile_one(s, level, cursor)
        self.in_streaming_loop = prev_streaming

        # ── Before %: prepare for corridor ──
        if has_advance:
            # G-restore: move H1 back to (0,0), then restore stream pos from _h1save.
            # This ensures _h1save always receives flat(0,0) after G, keeping
            # the G-save/restore cycle consistent across iterations.
            self._move_h1_to(DATA_ROW, 0, emit_here)
            h1save_col = self._var_col('_h1save')
            self._move_h0_to(DATA_ROW, h1save_col, emit_here)
            emit_here('G')   # swap(H1_register, grid[_h1save])
            # After G: H1 has stream pos (from _h1save), _h1save has flat(0,0).
            # Compiler can't track H1 statically (runtime-dependent stream pos).
            self.h1 = None
        ...
        # We need to reset H0 from end-of-body back to entry.
        # CL is already at var_col on both sides.
        h0_ops = self._head_move_ops(h0_end, h0_entry, 'E', 'W', 'S', 'N')

        if has_advance:
            # STREAMING: don't reset H1. The G-restore before % set H1 to
            # the stream position (runtime-dependent). The corridor must NOT
            # move H1 — it stays at the advanced stream pos for next read.
            h1_ops = []
        else:
            # Normal loop: reset H1 from end-of-body back to entry position.
        ...
        # ── Post-exit state ──
        if has_advance:
            # After a streaming loop exits, H1's position is runtime-dependent
            # (depends on how many iterations ran). We can't track it statically.
            self.h1 = None
            self.h1_stream = None
            # H0 is at h0_end (from body compilation)
        else:
            ...
            pass
        # (self.h0 is already at h0_end from body compilation)
...
""",
    ...
    'delta': {
        'source': """\
// delta-encode: transforms input bytes to deltas
// Double-read avoids 'zero' inside loop (which pollutes EX trail).
var n = 8
var curr = 0
var prev = 0
var delta = 0
...
""",
        'expected': {'n': 0, 'curr': 0, 'prev': 12, 'delta': 0},
        # EX trail: first output at col 1, then breadcrumb at col 2,
...
def run_test(name, verbose=False):
    ...
    # Check EX trail output at specific positions if expected
    expected_positions = test.get('expected_gp_trail_positions')
    if expected_positions is not None:
        ex_row = compiler.ex_row
        trail_ok = True
        for gp_col, exp_val in sorted(expected_positions.items()):
            got = cell_to_payload(sim.grid[ex_row * cols + gp_col])
            if got != exp_val:
                trail_ok = False
                print(f"  EX trail[{gp_col}]: expected={exp_val}, got={got} FAIL")
        if trail_ok:
            # Show just the output values in order
            vals = [expected_positions[k] for k in sorted(expected_positions)]
            print(f"  EX trail outputs: {vals} ok")
        else:
            ok = False
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: ifbc-03.py <source.ifb> [output.fb2d]")
        print("       ifbc-03.py --test [test_name]")
        print("       ifbc-03.py --test-all")
