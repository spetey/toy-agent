...
ARCHITECTURE:
  ...
  Branch: after prefix, GP points at EV which has p_all in bit 3 (rotated
  from bit 0 so it's visible to GP-conditional mirrors via DATA_MASK).
    - [GP] payload=0 (clean): # mirror triggers → fast path
    - [GP] payload≠0 (error): # mirror passes through → correction
...
class GadgetBuilder:
    """Build opcode sequences tracking head positions on GP row."""
...
def build_prefix():
    ...
    After prefix:
      H0 = EV(0), H1 = CWL(2), CL = ROT(8) payload=0, GP = EV(0)
    ...
    # Rotate EV left 3: bit0 → bit3 (data position visible to GP mirrors)
    gb.move_h0_col(DSL_EV)
    gb.emit_n('l', 3)
...
def build_correction():
    ...
    Entry state: H0=EV(0), H1=CWL(2), CL=ROT(8) payload=0, GP=EV(0)
                 [EV] has p_all in bit3 (rotated by prefix)
    Exit state:  H0=CWL(2), H1=CWL(2), CL=ROT(8) payload=0, GP=EV(0)
...
def build_fast_cleanup():
    ...
    Entry: H0=EV(0), H1=CWL(2), CL=ROT(8) payload=0, GP=EV(0), [EV]=0
    Exit:  H0=CWL(2), H1=CWL(2), CL=ROT(8) payload=0, GP=EV(0)
...
def make_fastpath_torus(cases, first_cw_col=4):
    ...
    max_gp_col = gp_start_col + DSL_ROT + n
...
def run_fastpath_test(cases, verbose=True, check_reverse=True):
    ...
    if check_reverse and n == 1:
        ...
        for col in range(sim2.cols):
            v = sim2.grid[sim2._to_flat(GP_ROW, col)]
            if v != 0:
                reverse_ok = False
                if verbose:
...
def generate_fb2d_files():
    ...
    print(f"          Row 2=CODE (prefix + # + correction), Row 3=GP (scratch)")
