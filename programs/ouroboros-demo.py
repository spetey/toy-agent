#!/usr/bin/env python3
"""
ouroboros-demo.py — Self-correcting ouroboros: 1 IP, 2 code rows eating each
other's errors.

The IP zigzags: east on row 0 (correcting row 1), west on row 1 (correcting
row 0). Each east+west cycle corrects ONE column on both rows, then H2
...
ARCHITECTURE:
  ...
  Row 2:         GP scratch (slides east on both passes)
...
ADVANCE:
  East pass:  E e ] > H     (5 ops: H0/H1/GP/CL east, H2 north to row 0)
  West pass:  E e ] > a h   (6 ops: H0/H1/GP/CL east, H2 east + south to row 1)

  East advance does NOT move H2 horizontally — just switches row.
  West advance moves H2 east by 1 AND switches row.
...
WHY NO V_PROBE:
  The original design used V (swap [CL]↔[H2]) for boundary detection: save
  the next target cell into CL, test it with conditional mirrors. But this
  caused a fatal reversibility bug: when H2 pointed at the V opcode cell
...
GP BEHAVIOR:
  Both passes slide the GP slot east. Each correction consumes 1 GP advance.
  After N cycles (2N corrections), GP has advanced 2N cells east.

Run tests:  python3 programs/ouroboros-demo.py
Save demo:  python3 programs/ouroboros-demo.py --save
"""
...
def make_ouroboros_torus(n_cycles, errors=None):
    ...
    # Grid width: code is CODE_WIDTH cols. GP needs room for 2*n_cycles advances.
    ...
    gp_start = 0   # GP slot starts at col 0 on GP row
    sim.h0 = sim._to_flat(GP_ROW, gp_start + DSL_CWL)
    sim.h1 = sim._to_flat(GP_ROW, gp_start + DSL_CWL)
    sim.h2 = sim._to_flat(WEST_ROW, 0)   # H2 starts at row 1, col 0
    sim.cl = sim._to_flat(GP_ROW, gp_start + DSL_ROT)
    sim.gp = sim._to_flat(GP_ROW, gp_start + DSL_EV)
...
def run_ouroboros_test(errors, n_cycles, label="", check_reverse=True):
    ...
    # Check corrected cells — H2 has visited cols 0..n_cycles-1 on both rows
    all_ok = True
    for row, col, bit in errors:
        if col < n_cycles:
            flat = sim._to_flat(row, col)
