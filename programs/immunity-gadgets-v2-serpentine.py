#!/usr/bin/env python3
"""
immunity-gadgets-v2-serpentine.py — Dual ouroboros with serpentine IX scanning.

Two identical gadgets correcting each other's code, wrapped at configurable width W.
Uses IX momentum ops (A/B/U) for serpentine scanning: east across row, south,
...
ARCHITECTURE:
  ...
  Gadget A waste:    row R+2 (EX lives here, eats zeros, excretes waste)
...
IX SCANNING (serpentine):
  Each IP's IX sweeps east across a row, detects boundary (zero cell),
...
BOUNDARY DETECTION (local-only, no remote cell modification):
  After A (advance IX), m copies [IX] to local [H0] via XOR (H0 was 0),
  ...
  C replaces old 'h' — advances IX in ix_vdir instead of hardcoded south.
...
VERTICAL BOUNDARY TEST (on code row, between & gates):
  ...
  Then m T ? T m tests [IX] at the vertically-advanced position.
  ...
  This makes IX ping-pong between first code row and handler row.

  Handler row is X-filled so IX includes it in the scan (gets corrected!).

  OLD DESIGN used V (swap [CL]↔[IX]) for boundary detection. This
...
Run tests:  python3 programs/immunity-gadgets-v2-serpentine.py [--width W]
Save demo:  python3 programs/immunity-gadgets-v2-serpentine.py --save [--width W]
"""
...
def build_serpentine_gadget(last_row_dir):
    """Build contained correction gadget with serpentine IX advance.

    CONTAINED DESIGN: H0/H1/CL stay at fixed DSL window positions on the
    stomach row across cycles. EX lives on the waste row below, eating
    fresh zeros and excreting waste. Only IX advances (serpentine).
    ...
    Preamble: T Z ] — deposits handler signal to waste row.
      ...
      Z swaps [H0]↔[EX] (H0=CWL on stomach, EX on waste row). Handler
      signal goes to waste row; fresh zero comes to CWL.
      ] advances EX east past the deposited waste.
      On clean cycles: all values are 0, so T and Z are 0↔0 no-ops.
      EX still advances (eats a zero that stays zero).

    Phase G waste deposit (after Phase F, H0=EV, H1=PA):
      Z deposits EV waste to waste row (swap [H0=EV]↔[EX=waste]).
      ] advances EX. E moves H0 to PA.
      Z deposits PA waste to waste row.
      ] advances EX. E moves H0 to CWL, e moves H1 to CWL.
      On clean cycles: EV=PA=0, Z swaps 0↔0, no drift.

    EX budget: 3 ] per cycle (1 preamble + 2 Phase G). With width W,
    EX wraps after W/3 cycles. Cheat mode refills waste row on wrap.
    ...
    # State after Phase F: H0=EV(0), H1=PA(1). EX on waste row.
    phase_g = [
        'Z',    # deposit EV waste: [H0=EV] ↔ [EX=waste_row]
        ']',    # EX advance east on waste row
        'E',    # H0: EV(0) → PA(1)
        'Z',    # deposit PA waste: [H0=PA] ↔ [EX=waste_row]
        ']',    # EX advance east on waste row
        'E',    # H0: PA(1) → CWL(2)
        'e',    # H1: PA(1) → CWL(2)
    ]
...
def compute_layout(width):
    ...
    layout = {
        ...
        'ga_waste': code_rows + 2,         # waste row (EX lives here)
...
def make_serpentine_ouroboros(width=99, errors=None):
    ...
    ga_waste = layout['ga_waste']  # waste row (EX lives here)
    gb_waste = layout['gb_waste']  # waste row

    # ── IP0: runs gadget A, IX starts on gadget B's first code row ──
    sim.ip_row = ga_start
    sim.ip_col = code_left
    sim.ip_dir = DIR_E
    sim.h0 = sim._to_flat(ga_gp, DSL_CWL)
    sim.h1 = sim._to_flat(ga_gp, DSL_CWL)
    sim.ix = sim._to_flat(gb_start, code_left)  # IX starts at gb code area
    sim.cl = sim._to_flat(ga_gp, DSL_ROT)
    sim.ex = sim._to_flat(ga_waste, 0)  # EX on waste row, col 0
    # ix_dir defaults to DIR_E (set in FB2DSimulator)

    # ── IP1: runs gadget B, IX starts on gadget A's first code row ──
    sim.add_ip(
        ip_row=gb_start, ip_col=code_left, ip_dir=DIR_E,
        h0=sim._to_flat(gb_gp, DSL_CWL),
        h1=sim._to_flat(gb_gp, DSL_CWL),
        ix=sim._to_flat(ga_start, code_left),
        cl=sim._to_flat(gb_gp, DSL_ROT),
        ex=sim._to_flat(gb_waste, 0),  # EX on waste row, col 0
    )
...
def _place_gadget(sim, layout, op_values, start_row):
    ...
    HANDLER (horizontal boundary):
      When IX's horizontal advance (A) hits a zero cell, the conditional
      mirror (? or !) in the boustrophedon fires, dropping IP south to
      the handler row.  Handler: / B C U : \\
        / redirects S→W.  B retreats IX horizontally (undo A).
        C advances IX vertically in ix_vdir (replaces old hardcoded 'h').
        U flips ix_dir.  : increments CL (signal for & gate).
    ...
    VERTICAL BOUNDARY TEST (on code row, between & gates):
      ...
      pattern as the preamble).  Then m T ? T m tests [IX]:
        m copies [IX] to H0 (XOR, H0 was 0).
        T moves H0 to CL for testing.
        ? (/ if CL==0) fires if IX is on a zero cell (outside code area).
        T m restore on the non-bounce path.

      On non-handler cycles: T Z are 0↔0 no-ops, ] advances EX,
      m T ? T m is harmless (IX on valid code cell → CL≠0 → no fire).

    BOUNCE SUB-HANDLER (vertical boundary):
      When the vertical ? fires, IP drops to the handler row:
        / D O C : \\
        D retreats IX (undo the C that went to invalid row).
        O flips ix_vdir (N↔S).
        C re-advances IX in the new direction (bounce back).
        : increments CL.  \\ sends W→N to & #2 merge gate.

      This makes IX ping-pong between the first code row and the handler
      row (inclusive), without ever entering stomach/waste rows.

    HANDLER ROW X-FILL:
      The handler row is filled with X (swap, no-op when H0=H1) so that
      IX sees nonzero cells there and includes it in the scan.  This means
      the handler row itself gets error-corrected by the sweep — desirable!
      IX's vertical boundary triggers on the stomach row (all zeros),
      not the handler row.
    """
    W = layout['width']
    R = layout['code_rows']
    ...
    # ── Fill handler row with X (so IX includes it in the scan) ──
    ...
    # When IX hits the west boundary at col 1, vertical advance checks
    # the adjacent row at col 1.  If that cell is zero, IX bounces
...
def _fill_row(sim, layout, row):
    ...
    Used for:
    - Last boustrophedon row: prevents false IX boundary detection.
    - Handler row: makes IX include it in the scan (gets error-corrected).
...
def simulate_h2_scan(layout, ip_idx, max_cycles):
    """Simulate IX serpentine movement with vertical ping-pong.

    IX scans horizontally (A), bouncing at row boundaries (B + U).
    On horizontal boundary: C advances IX vertically in ix_vdir.
    If that lands on a zero cell (outside code+handler area):
      D retreats, O flips ix_vdir, C re-advances (ping-pong bounce).
    Handler row is X-filled, so IX includes it in the scan.

    Args:
        layout: from compute_layout()
        ip_idx: 0 for IP0 (IX scans gb), 1 for IP1 (IX scans ga)
    ...
    h2_col = code_left
    ix_dir = DIR_E    # horizontal momentum
    ix_vdir = DIR_S   # vertical momentum (starts south)
    ...
    for cycle in range(max_cycles):
        # Record current IX position (cell to be corrected)
        cells.append((h2_row, h2_col))

        # ── Horizontal advance (A) ──
        new_row = (h2_row + dr[ix_dir]) % T
        new_col = (h2_col + dc[ix_dir]) % W

        if is_nonzero(new_row, new_col):
            # Normal: advance IX horizontally
            h2_row, h2_col = new_row, new_col
        else:
            # Horizontal boundary detected
            # B: retreat (don't commit the advance)
            # C: advance IX vertically in ix_vdir
            vert_row = (h2_row + dr[ix_vdir]) % T
            ...
            if is_nonzero(vert_row, h2_col):
                ...
                # D: retreat (don't commit), O: flip ix_vdir, C: re-advance
                ix_vdir = ix_vdir ^ 2  # O: flip N↔S
                bounce_row = (h2_row + dr[ix_vdir]) % T
                h2_row = bounce_row
...
def h2_cycle_for_cell(layout, ip_idx, target_row, target_col, max_cycles=2000):
    """Find which cycle IX visits a specific cell.
...
def early_code_cells(layout, ip_idx, max_cycle=None):
    """Return code cells visited within EX budget.
...
def _cheat_clear_waste(sim, layout):
    """Cheat mode: zero both waste rows so EX never encounters old waste.
...
def _step_all_infinite_zeros(sim, layout):
    ...
    after EVERY step_all(), so EX always sees clean zeros ahead.
...
def sweep_length(layout):
    """Calculate one full IX sweep (down-up or up-down) in cycles.

    A "sweep" = IX visits every cell in every code row + handler row,
    ...
    # (bottom row visited once, then IX bounces back up)
...
def run_serpentine_test(width, errors, label="", check_reverse=False,
    ...
    Args:
        cheat: if True, clear waste rows between cycles to prevent EX
               from encountering old waste on wrap. Non-reversible.
               If 'infinite', zero waste rows after every step_all() —
               this is reversible and has no EX budget limit.
    """
    if label:
        print(f"=== {label} ===")
    ...
    if check_reverse:
        ...
        # Limited to EX budget (~219 cycles for W=99), but the correction
...
def test_layout_info(width=99):
    ...
    print(f"    IP0 IX first code cells: "
...
def test_random_early(width=99, n_errors=8, seed=42):
    """Random errors on early scan cells (within EX budget)."""
    random.seed(seed)
    layout = compute_layout(width)
    ga_code = layout['ga_code']
    gb_code = layout['gb_code']

    # Use default max_cycle (EX budget) to avoid exceeding EX trail
...
def test_h2_pingpong(width=99):
    """Verify IX ping-pongs within code+handler rows only.

    Tests that IX never enters stomach or waste rows (which would
    cause irreversibility from encountering changing values).
    """
    print(f"=== IX ping-pong scan (W={width}) ===")
    layout = compute_layout(width)
    ...
    for ip_idx in [0, 1]:
        ...
        if escaped:
            print(f"    {label}: FAIL — IX escaped to rows {sorted(escaped)}")
            return False
    ...
    print(f"    IX bounded within code+handler rows: ok")
    return True
...
def test_long_reverse(width=99, n_cycles=200):
    ...
    Limited by EX wrapping contamination: handler deposits to waste every
    ~96 cycles; EX wraps at W/4 ≈ 25 cycles. At ~219 cycles for W=99,
    EX recycles old deposits → irreversibility. 200 cycles is safe.
...
def test_late_errors_cheat(width=99):
    ...
    zeros waste rows every step, so EX never encounters old waste.
    """
    layout = compute_layout(width)
...
def save_demo(width=99, filename=None):
    ...
    if filename is None:
        filename = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                f'immunity-gadgets-v2-serpentine-w{width}.fb2d')
    ...
    print(f"  Load: immunity-gadgets-v2-serpentine-w{width}")
