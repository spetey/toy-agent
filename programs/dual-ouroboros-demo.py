...
2-BIT DETECTION (inserted between Phase D and Phase C'):
  ...
  Step 2: Copy neighbor via H2 jump:
          h*delta (H2 to neighbor row)
          m: NBR ^= [H2] (copy neighbor value)
          H*delta (H2 back)
...
SLOT LAYOUT:
  ...
  Advance = SLOT_WIDTH (11 steps per head on GP, 1 step H2).
...
def build_h2_correction_gadget_2bit(neighbor_delta_rows=3):
    """Build H2 correction gadget with 2-bit neighbor copy.
    ...
    if neighbor_delta_rows > 0:
        gb.emit_n('h', neighbor_delta_rows)   # H2 south
    else:
        gb.emit_n('H', -neighbor_delta_rows)  # H2 north

    gb.emit('m')                   # [H0=NBR] ^= [H2=neighbor]

    # Move H2 back
    if neighbor_delta_rows > 0:
        gb.emit_n('H', neighbor_delta_rows)   # H2 north (back)
    else:
    ...
    # ── Advance: all heads east by SLOT_WIDTH on GP, H2 east by 1 ──
    gb.emit_n('E', DSL2_SLOT_WIDTH)    # H0 east ×11
    gb.emit_n('e', DSL2_SLOT_WIDTH)    # H1 east ×11
    gb.emit_n(']', DSL2_SLOT_WIDTH)    # GP east ×11
...
def make_2bit_test_torus(cases, first_cw_col=2, neighbor_delta=3):
    ...
    H2 starts on row 0. Neighbor is 3 rows south (delta=3).

    cases: list of (payload, error_bits)
        error_bits: None (clean), int (1-bit), or (int,int) (2-bit)

    Returns: (sim, expected_results, steps_per_cycle, gp_start_col)
    """
    ...
    # GP scratch layout
    gp_start_col = first_cw_col - DSL2_CWL
    max_gp_col = gp_start_col + (n - 1) * DSL2_SLOT_WIDTH + DSL2_NBR
    ...
    sim.h0 = sim._to_flat(GP_ROW_LINEAR, gp_start_col + DSL2_CWL)
    sim.h1 = sim._to_flat(GP_ROW_LINEAR, gp_start_col + DSL2_CWL)
    sim.h2 = sim._to_flat(TARGET_ROW, first_cw_col)
    sim.cl = sim._to_flat(GP_ROW_LINEAR, gp_start_col + DSL2_ROT)
    sim.gp = sim._to_flat(GP_ROW_LINEAR, gp_start_col + DSL2_EV)

    # Cycle length = cols (IP wraps around torus)
    steps_per_cycle = cols

    return sim, expected, steps_per_cycle, gp_start_col
...
def run_2bit_test(cases, verbose=True, check_reverse=True,
    ...
    h0_exp = sim._to_flat(gp_row, final_slot_start + DSL2_CWL)
    h1_exp = sim._to_flat(gp_row, final_slot_start + DSL2_CWL)
    gp_exp = sim._to_flat(gp_row, final_slot_start + DSL2_EV)
    cl_exp = sim._to_flat(gp_row, final_slot_start + DSL2_ROT)
    ...
    if verbose or not heads_ok:
        ...
        if not heads_ok:
            print(f"      Expected: H0={final_slot_start + DSL2_CWL}"
                  ...
                  f" GP={final_slot_start + DSL2_EV}"
                  f" CL={final_slot_start + DSL2_ROT}"
                  f" H2=(0,{first_cw_col + n})")
    all_ok &= heads_ok
    ...
    if check_reverse:
        ...
        for col in range(sim.cols):
            ...
            if v != 0:
                reverse_ok = False
                if verbose:
                    print(f"    [REVERSE] GP col {col}: 0x{v:04x}")
                break
...
def save_demo(payload=42, error_bits=(3, 7), filename=None):
    ...
    print(f"    Row 0: TARGET (codewords to correct, H2 scans here)")
    print(f"    Row 1: CODE   (gadget opcodes, IP goes east)")
    print(f"    Row 2: GP     (scratch cells)")
    ...
    print()
    print(f"  H0,H1 at (2,{gp_start + DSL2_CWL}) on GP row (CWL slot)")
    print(f"  H2 at (0,2) on TARGET row")
    print(f"  CL at (2,{gp_start + DSL2_ROT}) on GP row (ROT slot)")
    print(f"  GP at (2,{gp_start + DSL2_EV}) on GP row (EV slot)")
    print()
