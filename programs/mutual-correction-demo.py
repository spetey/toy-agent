...
(Gadget A on IP0, Gadget B on IP1) each using the H2 copy-down pattern to
correct the other's code. They share the grid but have independent heads.

LAYOUT (4 rows × 325 cols, linear):

  Row 0: Gadget A code  (IP0 executes, IP1's H2 corrects)
  Row 1: Gadget A GP    (IP0's scratch: H0, H1, CL, GP here)
  Row 2: Gadget B code  (IP1 executes, IP0's H2 corrects)
  Row 3: Gadget B GP    (IP1's scratch)
...
CYCLE MATH:
  ...
  GP scratch: ~36 cycles max (325 / DSL_SLOT_WIDTH)
...
def make_mutual_torus(errors_a=None, errors_b=None):
    ...
    gp_start_col = FIRST_CW_COL - DSL_CWL  # = 0
    ...
    # ── IP0 (Gadget A): executes row 0, H2 scans row 2 ──
    sim.ip_row = ROW_A_CODE
    sim.ip_col = 0
    sim.ip_dir = DIR_E
    sim.h0 = sim._to_flat(ROW_A_GP, gp_start_col + DSL_CWL)
    sim.h1 = sim._to_flat(ROW_A_GP, gp_start_col + DSL_CWL)
    sim.h2 = sim._to_flat(ROW_B_CODE, FIRST_CW_COL)  # ← scans B
    sim.cl = sim._to_flat(ROW_A_GP, gp_start_col + DSL_ROT)
    sim.gp = sim._to_flat(ROW_A_GP, gp_start_col + DSL_EV)
    sim._save_active()

    # ── IP1 (Gadget B): executes row 2, H2 scans row 0 ──
    sim.add_ip(
        ip_row=ROW_B_CODE, ip_col=0, ip_dir=DIR_E,
        h0=sim._to_flat(ROW_B_GP, gp_start_col + DSL_CWL),
        h1=sim._to_flat(ROW_B_GP, gp_start_col + DSL_CWL),
        h2=sim._to_flat(ROW_A_CODE, FIRST_CW_COL),  # ← scans A
        cl=sim._to_flat(ROW_B_GP, gp_start_col + DSL_ROT),
        gp=sim._to_flat(ROW_B_GP, gp_start_col + DSL_EV),
    )
...
def verify_gp_clean(sim, gp_row, cols):
    """Check GP row is all zeros. Returns True if clean."""
    for c in range(cols):
        v = sim.grid[sim._to_flat(gp_row, c)]
        if v != 0:
            print(f"    GP row {gp_row} col {c}: 0x{v:04x} != 0")
            return False
    return True
...
def test_a_corrects_b():
    """A is clean, B has parity-only errors. IP0 corrects B via H2.
    ...
    executes identically to correct code. IP0's H2 detects and fixes
    the parity errors.
    """
...
def test_b_corrects_a():
    """B is clean, A has parity-only errors. IP1 corrects A via H2."""
...
def test_mutual_correction():
    ...
    their code identically to clean code. Meanwhile, each IP's H2 detects
...
def save_demo(filename=None, errors_b=None):
    ...
    print(f"  Row 1: Gadget A GP (IP0 scratch)")
    print(f"  Row 2: Gadget B code (IP1 executes, {len(errors_b)} error(s))")
    print(f"  Row 3: Gadget B GP (IP1 scratch)")
    print(f"  2 IPs: IP0 at (0,0)→E, IP1 at (2,0)→E")
    print(f"  IP0's H2 at (2,{FIRST_CW_COL}) — corrects B")
    print(f"  IP1's H2 at (0,{FIRST_CW_COL}) — corrects A")
