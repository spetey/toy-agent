#!/usr/bin/env python3
"""
immunity-gadgets-v8-correction-mask.py -- Copy-over for 2-bit errors.
...
Run tests:  python3 programs/immunity-gadgets-v8-correction-mask.py [--width W]
"""
...
def build_probe_bypass_gadget(last_row_dir):
    ...
    gb.emit(']')     # advance EX to clean cell for Phase G
    gb.emit('T')     # undo T: restore CL↔EV
    gb.emit_n('r', 3)  # undo lll rotation: EV bit3 → bit0

    # Undo Phase B z-extract: EV has p_all residual at bit0 (=1 for 1-bit
    # errors).  Must clean EV before V, which XORs INTO it.
    gb.move_h1_col(DSL_PA)      # H1: CWL → PA
    gb.emit('z')                 # swap bit0 EV↔PA: EV bit0=0, PA bit0=p_all
    gb.move_h1_col(DSL_CWL)     # H1: PA → CWL (needed for Phase A' Y ops)

    # Phase A': uncompute PA (Y-undo, CL from 15 → 0)
    gb.move_h0_col(DSL_PA)      # H0: EV → PA
    gb.xor_accumulate_bits(list(range(15, -1, -1)))

    # V correction: [EV] ^= (1 << syndrome([IX]))
    # Replaces Phases C+D+C' (~160 ops) with a single opcode.
    # The pre-syndrome filter guarantees only 1-bit errors reach here
    # (2-bit errors → copy-over, clean cells → bypass).  In standard-
    # form Hamming, syndrome = error bit position, so 1 << syndrome is
    # the exact correction mask.  EV is 0 (cleaned by z undo above).
    gb.move_h0_col(DSL_EV)      # H0: PA → EV
    gb.emit('V')     # [EV] ^= (1 << syndrome([IX]))

    # Uncompute copy-in + writeback
    gb.move_h0_col(DSL_CWL)
    gb.emit('m')     # CWL ^= [IX] → 0
    gb.move_h0_col(DSL_EV)
    gb.emit('j')     # [IX] ^= EV → corrected!
...
def test_build_gadget(width=195):
    print(f"=== Build gadget v8 (W={width}) ===")
    try:
...
def test_cycle_length(width=195):
    print(f"=== Cycle length v8 (W={width}) ===")
...
def test_single_error(width=195):
    print(f"=== Single error v8 (W={width}) ===")
    layout = compute_probe_layout(width)
...
def test_copyover(width=195):
    print(f"=== Copy-over (2-bit) v8 (W={width}) ===")
    layout = compute_probe_layout(width)
    gb_start = layout['gb_code'][0]
    code_left = layout['code_left']
...
if __name__ == '__main__':
    ...
    if all_ok:
        ...
        out_path = os.path.join(out_dir, f'immunity-gadgets-v8-correction-mask-w{width}.fb2d')
        sim.save_state(out_path, hints={'waste_cleanup': 1})
        print(f"Saved: {out_path}")
        print("=" * 60)
        print(f"ALL v8 TESTS PASSED (W={width})")
        print("=" * 60)
    else:
        print("=" * 60)
