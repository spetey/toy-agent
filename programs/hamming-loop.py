...
GRID LAYOUT:
  ...
  Row 4+N+1:         GP — scratch cells (PA, SYND, S0..ROT)

The gadget's heads (H0, H1, CL, GP) point into rows 0 and GP_ROW.
...
def compute_layout(left_col, right_col, code_start_row):
    ...
    Returns: (code_ops, op_values, n_rows, gp_row, code_rows)
    """
    ...
    for _ in range(10):
        ...
        gp_row = code_start_row + code_rows
        new_n_rows = gp_row + 1
        new_gp_dist = gp_row  # GP_ROW - DATA_ROW(0) = gp_row
    ...
    return code_ops, op_values, n_rows, gp_row, code_rows
...
def make_constrained_gadget(codeword):
    ...
    Data on row 0. GP scratch on the row after the last code row.
    """
    code_ops, op_values, n_rows, gp_row, code_rows = compute_layout(
...
def run_constrained_test(data4, error_bit=None, verbose=False):
    ...
    sim, gp_row = make_constrained_gadget(bad)
...
def save_constrained_fb2d():
    ...
    for name, data4, err_bit in cases:
        ...
        sim, gp_row = make_constrained_gadget(cw)
