...
class FBSimulator:
    ...
    def step_back(self) -> bool:
        ...
        This is TRUE reversibility - no history needed!
        
        The key insight: we need to find where IP came from, then undo that instruction.
        - For most instructions: prev_IP = current_IP - 1, and we apply the inverse op
        - For J: we came from where tape[CL] points (since J swapped IP with tape[CL])
        
        Returns True always (reversible computation can always go back).
        """
        prev_ip_linear = (self.ip - 1) % self.tape_size
        
        # Check if we came from J
        j_candidate = self.tape[self.cl]  # If we came from J, it was here
        
        came_from_j = False
        if 0 <= j_candidate < self.tape_size and self.tape[j_candidate] == OPCODES['J']:
            # There's a J at j_candidate. Did we come from it?
            prev_instr = self.tape[prev_ip_linear]
            
            if prev_instr == OPCODES['J']:
                # Previous linear position has J. 
                # If that's the same J as j_candidate, we came from it.
                # If different, the J at prev_ip_linear jumped elsewhere, so we came from j_candidate.
                came_from_j = True
            elif prev_instr == 0:
                # Previous linear position is NOP (opcode 0).
                # NOP just advances IP, so it's valid... but if there's a J pointing here,
                # it's more likely we came from J (NOPs are often unused tape).
                # Heuristic: prefer J if j_candidate != prev_ip_linear + 1
                # (i.e., J didn't just happen to land us at the next position)
                if j_candidate != (prev_ip_linear + 1) % self.tape_size:
                    came_from_j = True
            # else: prev_instr is a real non-J instruction, prefer normal flow
        
        if came_from_j:
            # Reverse J: swap IP and tape[CL] again (J is self-inverse)
            tmp = self.ip
            self.ip = self.tape[self.cl] % self.tape_size
            self.tape[self.cl] = tmp
        else:
            # Normal flow: we came from prev_ip_linear
            self.ip = prev_ip_linear
            
            # Undo the instruction at prev_ip_linear
            opcode = self.tape[prev_ip_linear]
            ...
            if opcode == OPCODES['<']:
                ...
                # Came from J via "normal flow" means J is at prev_ip_linear
                # and we decided not to reverse via j_candidate. Do J reversal.
                tmp = self.ip
                self.ip = self.tape[self.cl] % self.tape_size
                self.tape[self.cl] = tmp
            ...
            # else: NOP, nothing to undo (IP already moved back)
    ...
    while True:
        ...
        try:
            if command in ('quit', 'q', 'exit'):
                ...
                sim = FBSimulator(tape_size=sim.tape_size)  # Keep current tape size
