#!/usr/bin/env python3
"""
F***brain Tape-Swap Simulator v3 - Separated Condition/Target Jump
Authored or modified by Claude
Version: 2025-01-30 v3.0

This version implements:
- Conditional jump with SEPARATE condition (CL) and target (H1)
- Accumulator operations (. and ,)
- Sam Eisenstat's tape-swap approach for reversible jumps

State: tape[0..N-1], IP, CL, H0, H1 (all mod N)
  - IP = instruction pointer
  - CL = control locus (condition for J, pointer for S)
  - H0 = data head 0 (destination for +/-/./,)
  - H1 = data head 1 (jump target pointer for J, source for ./,)
...
ISA:
  ...
  , : tape[H0] -= tape[H1]                         (inverse: .)
  J : if tape[CL] != 0: tape-swap jump to tape[H1] (conditional, self-inverse)
  S : swap(tape[CL], tape[H0])                     (self-inverse)
  G : swap(CL, tape[H0])                           (self-inverse)

J (conditional jump) when tape[CL] != 0:
  1. target = tape[H1]
  2. swap(tape[IP], tape[target]) - move J to target location  
  3. swap(IP, tape[H1]) - swap addresses (puts return addr in tape[H1])
  4. IP += 1 - always increment (even when condition false)

Key insight: Condition (tape[CL]) is separate from target (tape[H1]).
After jump, tape[H1] holds the return address for reversibility.
"""
...
class FBTapeSwapSimulator:
    ...
    def step(self) -> bool:
        ...
        if opcode == OPCODES['<']:
            ...
            idx = self.h0 % self.tape_size
            self.tape[idx] = (self.tape[idx] + 1) & 0xFF
            self.ip = (self.ip + 1) % self.tape_size
            ...
            # Inverse accumulate: tape[H0] -= tape[H1]
            h0_idx = self.h0 % self.tape_size
            h1_idx = self.h1 % self.tape_size
            self.tape[h0_idx] = (self.tape[h0_idx] - self.tape[h1_idx]) & 0xFF
            self.ip = (self.ip + 1) % self.tape_size
            ...
            # Conditional Tape-swap Jump (only if tape[CL] != 0):
            # Condition: tape[CL], Target: tape[H1]
            # 1. target = tape[H1]
            # 2. swap(tape[IP], tape[target]) - move J to target location
            # 3. swap(IP, tape[H1]) - swap addresses (stores return addr in tape[H1])
            # 4. IP += 1 - always increment (even if condition false)
            
            if self.tape[self.cl % self.tape_size] != 0:
                target = self.tape[self.h1 % self.tape_size] % self.tape_size
                
                # Step 1: swap tape values (move J to target)
                tmp = self.tape[self.ip]
                self.tape[self.ip] = self.tape[target]
                self.tape[target] = tmp
                
                # Step 2: swap IP with tape[H1] (stores return address)
                tmp = self.ip
                self.ip = self.tape[self.h1 % self.tape_size] % self.tape_size
                self.tape[self.h1 % self.tape_size] = tmp
            
            # Step 3: always increment IP (even if we didn't jump)
            self.ip = (self.ip + 1) % self.tape_size
            ...
            # Note: CL will be modded when USED as index, not here
            tmp = self.cl
            self.cl = self.tape[self.h0 % self.tape_size]  # don't mod the value!
            self.tape[self.h0 % self.tape_size] = tmp
            self.ip = (self.ip + 1) % self.tape_size
    ...
    def step_back(self) -> bool:
        """
        Reverse one instruction using tape[IP-1] to determine what just executed.
        
        Key insight: tape[IP-1] always contains the instruction that just ran.
        - If it's J, we jumped here and need to reverse the tape swap
        - Otherwise, we stepped here normally
        
        TODO: Implement full reversal logic.
        """
        print("step_back not yet fully implemented for tape-swap simulator")
        
        prev_pos = (self.ip - 1) % self.tape_size
        prev_instr = self.tape[prev_pos]
        
        print(f"  tape[IP-1] = tape[{prev_pos}] = {prev_instr} ({OPCODE_TO_CHAR.get(prev_instr, '?')})")
        
        if prev_instr == OPCODES['J']:
            print("  -> Previous instruction was J (jump)")
        else:
            print("  -> Previous instruction was not J (normal step)")
...
def interactive_session():
    ...
    sim = FBTapeSwapSimulator(tape_size=DEFAULT_TAPE_SIZE)
    
    print("=" * 70)
    print("F***brain Tape-Swap Simulator - Reversible Brainf*** - Interactive Mode")
    print("=" * 70)
    print(f"""
...
ISA:
  ...
  x  swap tape[H0],tape[H1]         .  tape[H0] += tape[H1]
  ,  tape[H0] -= tape[H1]           J  if tape[CL]: jump to tape[H1]
  S  swap tape[CL],tape[H0]         G  swap CL,tape[H0]
...
Commands:
  ...
  back / b / r          Reverse one instruction
    ...
    while True:
        ...
        try:
            if command in ('quit', 'q', 'exit'):
                                ...
                                if '=' in line:
                                    ...
                                    if key == 'tape_size':
                                        ...
                                        if new_size != sim.tape_size:
                                            sim = FBTapeSwapSimulator(tape_size=new_size)
                                    elif key == 'ip':
                                        sim.ip = int(val)
                                    elif key == 'cl':
                                        sim.cl = int(val)
                ...
                sim = FBTapeSwapSimulator(tape_size=sim.tape_size)  # Keep current tape size
