#!/usr/bin/env python3
"""
RBF Simulator - Reversible BrainF
Authored or modified by Claude
Version: 2025-01-27 v1.0

RBF is a reversible computation language inspired by BFF, using Fredkin-style
conditional swaps instead of irreversible brackets.

State: tape[0..15], IP, CL, H0, H1 (all mod 16)
  - IP = instruction pointer
  - CL = control locus (condition register for F, swap target for J)
...
class RBFSimulator:
    ...
    def load_program(self, code: str, data_offset: Optional[int] = None,
        ...
        """
        Load an RBF program onto the tape.
        
        code: string of RBF instructions (< > { } ( ) + - x F J S)
    ...
    def step(self) -> bool:
        ...
        # Save state for history (enabling future reverse execution)
        self.history.append({
            'tape': self.tape.copy(),
            'ip': self.ip,
            'cl': self.cl,
        ...
        if opcode == OPCODES['<']:
            ...
            self.tape[self.cl] = (tmp + 1) & 0xFF  # Store return address (next instruction)
...
def interactive_session():
    """Run an interactive RBF session."""
    sim = RBFSimulator(tape_size=16)
    
    print("=" * 60)
    print("RBF Simulator - Reversible BrainF - Interactive Mode")
    print("=" * 60)
    print("""
State: tape[0..15], IP, CL, H0, H1 (all mod 16)
...
Instructions:
  ...
  ( ) : H1 -= 1 / H1 += 1
  + - : tape[H0] += 1 / tape[H0] -= 1
  x   : swap(tape[H0], tape[H1])
  F   : if tape[CL] != 0: swap(tape[H0], tape[H1])
  J   : swap(IP, tape[CL])  [stores return addr]
  S   : swap(tape[CL], tape[H0])

Commands:
  load <code>           Load RBF program
    ...
    while True:
        ...
        try:
            if command in ('quit', 'q', 'exit'):
                ...
                print("Commands: load, data, cl, h0, h1, step, back, run, compact, reset, quit")
                sim.display(compact=compact_mode)
