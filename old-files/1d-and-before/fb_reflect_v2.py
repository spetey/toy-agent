#!/usr/bin/env python3
"""
F***brain Reflective Simulator v2 - Bouncing IP with Mirrors
...
ISA:
  ...
  | (15) : DIR := -DIR                             (self-inverse, unconditional wall)
  / (16) : if tape[CL]!=0: DIR := -DIR             (self-inverse, reflect on nonzero)
  \\ (17) : if tape[CL]==0: DIR := -DIR             (self-inverse, reflect on zero)
  ^ (18) : if DIR==+1: tape[H0]++                  (inverse: v)
  v (19) : if DIR==+1: tape[H0]--                  (inverse: ^)
  [ (20) : if DIR==-1: tape[H0]++                  (inverse: ])
  ] (21) : if DIR==-1: tape[H0]--                  (inverse: [)
...
OPCODES = {
    ...
    'S': 13,
    'G': 14,
    '|': 15,   # wall (unconditional reflect)
    '/': 16,   # mirror: reflect if tape[CL] != 0
    '\\': 17,  # mirror: reflect if tape[CL] == 0
    '^': 18,   # if DIR==+1: tape[H0]++
    'v': 19,   # if DIR==+1: tape[H0]--
    '[': 20,   # if DIR==-1: tape[H0]++
    ']': 21,   # if DIR==-1: tape[H0]--
}
...
class FBReflectSimulator:
    ...
    def step_back(self) -> bool:
        ...
        # Determine prev_DIR
        if opcode == OPCODES['|']:
            # | always flips, so prev_DIR was opposite
            prev_dir = -self.dir
        elif opcode == OPCODES['/']:
            # / reflects if tape[CL] != 0
            cl_idx = self.cl % self.tape_size
            if self.tape[cl_idx] != 0:
        ...
        if opcode == OPCODES['>']:
            ...
            # Was: if DIR==+1: tape[H0]++
            # prev_dir tells us if it acted
            if prev_dir == 1:
                h0_idx = self.h0 % self.tape_size
                self.tape[h0_idx] = (self.tape[h0_idx] - 1) & 0xFF
        ...
        # |, /, \ don't modify tape/registers (only DIR), and we've handled DIR above
        # NOP: nothing to undo
...
def interactive_session():
    ...
    print("=" * 70)
    print("F***brain Reflective Simulator v2 - Bouncing IP with Mirrors")
    print("=" * 70)
    print(f"""
...
ISA (opcode in parens):
  ...
  | (15) flip DIR              (wall)
  / (16) if tape[CL]!=0: flip DIR  (mirror)
  \\ (17) if tape[CL]==0: flip DIR  (mirror)
  ^ (18) if DIR→: tape[H0]++   v (19) if DIR→: tape[H0]--
  [ (20) if DIR←: tape[H0]++   ] (21) if DIR←: tape[H0]--
        ...
        try:
            ...
            if cmd in ('quit', 'q'):
                ...
                if len(parts) >= 3:
                    ...
                    for v in parts[2:]:
                        if v in OPCODES:
                            values.append(OPCODES[v])
                        else:
                            values.append(int(v) & 0xFF)
                    for i, val in enumerate(values):
                        sim.tape[(offset + i) % sim.tape_size] = val
...
Execution: read tape[IP], execute, IP += DIR
Reversibility: condition value at /\\ disambiguates path taken

Commands:
  tape <code>       Load program onto tape (resets state)
  data <pos> <v>... Set tape values at position (v can be number or symbol)
