...
ARCHITECTURE:
  ...
  Row 3:  EX     — scratch cells (all zero)

CODE ROW LAYOUT:
  Col 1:  % (return entry: / reflect if CL!=0 → N→E)
  Col 2:  V (restore: swap CL ↔ [IX], restoring probed cell)
...
V SAVE/RESTORE:
  V_probe (end of pass): swap CL ↔ [IX]. Saves next cell into CL, leaves [IX]=0.
  V_restore (start of next pass): swap CL ↔ [IX]. Restores cell, CL=0 for correction.
...
EX BEHAVIOR:
  The correction cycle's sliding window advances EX by 1 per correction.
  After all target cells corrected, idle passes continue advancing EX (no-op
  corrections consume EX cells). This is the "EX burn" cost of the idle state.
...
def build_contained_gadget_ops():
    ...
    # Col offset 1: V (restore [IX] from CL)
    ops.append('V')    # col 2
    ...
    # Advance: E e ] > a (H0, H1, EX, CL east; IX east)
    ops.extend(['E', 'e', ']', '>', 'a'])

    # V probe: CL = [next_H2_cell], [IX] = 0
    ops.append('V')
...
def make_contained_torus(cases, first_cw_col=FIRST_CW_COL):
    ...
    # EX scratch: starts at first_cw_col - DSL_CWL, extends n + DSL_ROT cells
    ex_start_col = first_cw_col - DSL_CWL
    gp_end_col = ex_start_col + n + DSL_ROT

    # Grid width: fit code, target, and EX scratch
    grid_width = max(code_end_col + 4,      # code + small padding
                     first_cw_col + n + 2,   # target cells + sentinel
                     gp_end_col + 2)         # EX scratch
    ...
    return sim, expected, cycle_length, ex_start_col, first_pass_steps
...
def run_contained_test(cases, verbose=True, check_reverse=True):
    ...
    """
    n = len(cases)
    sim, expected, cycle_length, ex_start_col, first_pass_steps = \
