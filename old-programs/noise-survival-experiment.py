...
def run_one_cycle(sim, grid_width):
    ...
    One cycle processes ONE code cell via IX.
    A full IX sweep = grid_width cycles = grid_width² step_all calls.
    """
...
def run_one_sweep(sim, grid_width):
    """Run one full IX sweep = grid_width cycles = grid_width² step_all() calls.
...
def zero_gp_rows(sim, grid_width):
    """Zero EX rows (cheat to avoid dirty-trail interference)."""
...
def run_trial(noise_rate, max_sweeps=500, seed=None, verbose=False,
    ...
    ex_cleanup: if True, zero EX rows after each sweep (cheat mode).
    """
    ...
    for sweep in range(1, max_sweeps + 1):
        ...
        # Run one full IX sweep with gradual noise injection (per-cycle)
        total_injected = 0
...
def predict_mean_sweeps(n, n_cells=650, grid_width=325):
    ...
    One IX sweep = grid_width cycles. Noise is injected gradually
...
def run_experiment(rates, n_trials=30, max_sweeps=500, ex_cleanup=False):
    """Run experiment for multiple noise rates."""
    print("=" * 70)
    mode = "EX CLEANUP ON" if ex_cleanup else "NO EX CLEANUP"
