...
IP0 runs gadget A, correcting gadget B via IX.
IP1 runs gadget B, correcting gadget A via IX.
Both use IDENTICAL code — only initial head positions differ.

LAYOUT (6 rows × CODE_WIDTH cols):
  Row 0: IP0 east code (corrects row 4 via IX)
  Row 1: IP0 west code (corrects row 3 via IX)
  Row 2: IP0 EX scratch
  Row 3: IP1 east code (corrects row 1 via IX)  ← same code as row 0
  Row 4: IP1 west code (corrects row 0 via IX)  ← same code as row 1
  Row 5: IP1 EX scratch

ADVANCE (same as ouroboros):
  East pass:  E e ] > H      H0/H1/EX/CL east; IX north (target west→east)
  West pass:  E e ] > a h    H0/H1/EX/CL east; IX east + south (target east→west)

KEY PROPERTY:
  Both IPs use EXACTLY the same code. The correction code uses m/j to
  access [IX] without caring which row IX is on. The advance uses H/h
  (relative movement) which works from any starting row.

  IP0: IX oscillates between rows 3-4 (IP1's code)
  IP1: IX oscillates between rows 0-1 (IP0's code)
...
def make_dual_ouroboros(n_cycles, errors=None):
    ...
    # EX needs room for advances (1 cell per correction, 2 corrections per cycle)
    ...
    # ── IP0: runs on rows 0-1, IX scans rows 3-4 ──
    gp_start = 0
    sim.ip_row = IP0_EAST
    ...
    sim.ix = sim._to_flat(IP1_WEST, 0)  # IX starts at IP1's west row, col 0
    sim.cl = sim._to_flat(IP0_GP, gp_start + DSL_ROT)
    sim.ex = sim._to_flat(IP0_GP, gp_start + DSL_EV)

    # ── IP1: runs on rows 3-4, IX scans rows 0-1 ──
    ip1_idx = sim.add_ip(
        ...
        h2=sim._to_flat(IP0_WEST, 0),  # IX at IP0's west row, col 0
...
def run_dual_test(errors, n_cycles, label="", check_reverse=True):
    ...
    if check_reverse:
        ...
        # Check EX rows clean
        for ex_row in (IP0_GP, IP1_GP):
            for col in range(sim.cols):
                v = sim.grid[sim._to_flat(ex_row, col)]
                if v != 0:
                    reverse_ok = False
                    if label:
                        print(f"    [REVERSE] EX row {ex_row} col {col}: 0x{v:04x}")
                    break
