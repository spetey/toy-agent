#!/usr/bin/env python3
"""
boustrophedon-ouroboros-demo.py — Compact dual ouroboros with diagonal IX scanning.

Two identical gadgets correcting each other's code, wrapped at configurable width W.
Uses diagonal IX scanning (east+south per cycle) to cover all grid cells.

ARCHITECTURE:
  Gadget A code: rows 0..R-1 (boustrophedon at width W)
  Gadget A EX:   row R
  Gadget B code: rows R+1..2R (identical boustrophedon)
  Gadget B EX:   row 2R+1

  Total: T = (2R+2) rows × W cols

IX SCANNING (diagonal):
  Each IP's IX advances east+south per correction cycle (a h).
  After T×W cycles, every cell on the torus is visited exactly once.
  Requires gcd(W, T) = 1 for full coverage.

  Correcting non-code cells (zeros, EX scratch) is a no-op (syndrome=0).
...
IDENTICAL CODE:
  Both gadgets have identical boustrophedon code. IX advance is the same
...
EX CONSUMPTION:
  ...
  EX wraps. For full sweeps, use the server's EX cleanup feature or
...
def compute_layout(width):
    ...
    Returns dict with:
        ...
        ga_gp: row index for gadget A EX
        gb_code: (start_row, end_row) for gadget B code
        gb_gp: row index for gadget B EX
...
def make_boustrophedon_ouroboros(width=99, errors=None):
    ...
    ga_gp = layout['ga_gp']
    gb_gp = layout['gb_gp']
    gp_start = 0  # EX scratch starts at col 0

    # ── IP0: runs gadget A, IX scans starting at gadget B ──
    sim.ip_row = ga_start
    sim.ip_col = code_left   # first code op
    sim.ip_dir = DIR_E
    sim.h0 = sim._to_flat(ga_gp, gp_start + DSL_CWL)
    sim.h1 = sim._to_flat(ga_gp, gp_start + DSL_CWL)
    sim.ix = sim._to_flat(gb_start, 0)  # IX starts at gadget B row 0, col 0
    sim.cl = sim._to_flat(ga_gp, gp_start + DSL_ROT)
    sim.ex = sim._to_flat(ga_gp, gp_start + DSL_EV)

    # ── IP1: runs gadget B, IX scans starting at gadget A ──
    sim.add_ip(
        ...
        h2=sim._to_flat(ga_start, 0),  # IX at gadget A row 0, col 0
...
def h2_cell_at_cycle(k, start_row, start_col, T, W):
    """Return (row, col) that IX visits at cycle k."""
    return (start_row + k) % T, (start_col + k) % W


def cycle_for_cell(target_row, target_col, start_row, start_col, T, W):
    """Return the cycle number when IX visits (target_row, target_col).
...
def run_boustrophedon_test(width, errors, label="", check_reverse=False):
    ...
    # IP0's IX starts at (gb_start, 0), IP1's IX starts at (ga_start, 0).
    ga_start = layout['ga_code'][0]
    gb_start = layout['gb_code'][0]
    ...
    for row, col, bit in errors:
        ...
        if ga_start <= row <= layout['ga_code'][1]:
            # Cell on gadget A code → corrected by IP1's IX
            h2_start_row = ga_start
        elif gb_start <= row <= layout['gb_code'][1]:
            # Cell on gadget B code → corrected by IP0's IX
            h2_start_row = gb_start
        else:
            # Error on EX row — corrected by either IP as a no-op
            continue
...
# ═══════════════════════════════════════════════════════════════════
# Helpers: cells reachable within EX budget
# ═══════════════════════════════════════════════════════════════════

def early_cells(layout, ip_idx, max_cycle=None):
    """Return list of (cycle, row, col) for cells visited within EX budget.

    ip_idx: 0 for IP0 (IX starts at gb_code), 1 for IP1 (IX starts at ga_code)
    max_cycle: limit (default: width - DSL_ROT - 2, safe EX range)
    """
    T = layout['total_rows']
    W = layout['width']
...
def early_code_cells(layout, ip_idx, max_cycle=None):
    """Return only code cells (not EX rows) from early_cells."""
    T = layout['total_rows']
...
def test_mirror_cells(width=99):
    """Errors on mirror cells (boundary columns) within EX budget."""
    layout = compute_layout(width)
    T = layout['total_rows']
    # Find mirror/corridor cells reachable within EX budget
...
def save_demo(width=99, filename=None):
    ...
    print(f"  Gadget: {layout['n_ops']} ops, cycle={cycle_length} steps")
    print(f"  Gadget A code: rows {ga_start}-{ga_end}, EX: row {layout['ga_gp']}")
    print(f"  Gadget B code: rows {gb_start}-{gb_end}, EX: row {layout['gb_gp']}")
    ...
    print(f"  IP0: code rows {ga_start}-{ga_end},"
          f" EX row {layout['ga_gp']}, IX starts at ({gb_start},0)")
    print(f"  IP1: code rows {gb_start}-{gb_end},"
          f" EX row {layout['gb_gp']}, IX starts at ({ga_start},0)")
    print()
