...
Strategy for distinguishing code from data:
  ...
  - EX row: row containing gp initial position
...
def migrate_grid(state):
    ...
    for hr in [h0_row, h1_row]:
        if hr != ip_row:
            data_rows.add(hr)
    # EX row is scratch data
    if ex_row != ip_row:
        data_rows.add(ex_row)
...
def write_fb2d(filepath, state, new_grid):
    ...
    if n_ips > 1:
        ...
        for i in range(n_ips):
            prefix = f"ip{i}_"
            for field in ['ip_row', 'ip_col', 'ip_dir', 'cl', 'h0', 'h1', 'ix', 'ex']:
                key = f"{prefix}{field}"
                if key in raw:
        ...
        if state.get('ix', 0) != 0:
            lines.append(f"h2={state['ix']}")
        lines.append(f"gp={state['ex']}")
