#!/usr/bin/env python3
"""
fuel-demo.py — Agent that eats fuel, shuttles zeros to EX, does work.
...
ARCHITECTURE (4-row torus):
  ...
  Row 3 (EX):    0 0 0 0 0 0 0 0 0   ← EX trail
...
EACH LAP:
  ...
  Phase 2 — Shuttle zero to EX row:
    w       H1 west → back to fuel[2i] (the zero)
    S S S   H0 south x3 → H0 on EX row, col 2i
    E ...   H0 east to target EX cell (cell i)
    X       swap [H0] ↔ [H1] → EX cell gets 0, fuel[2i] gets old EX value

  Phase 3 — Restore H0 to fuel row for next pair:
    N N N   H0 north x3 → back to fuel row

  Phase 4 — Inner work using EX:
    ] ( P )  — advance EX, enter "loop" (1 iteration), leave breadcrumb
    (This is trivial work — just proves EX consumption works)
    Actually ] ( P ) is not right. Let's just do ] P — advance EX, write
    breadcrumb. This consumes one zero from the EX trail.
...
SHUTTLE DETAIL:
  ...
  Solution: H0 walks the EX row independently. Instead of going back to
  the fuel row between iterations, H0 STAYS on the EX row and advances
  ...
  NEW PLAN — fixed head roles:
    H0 = EX row walker (stays on row 3, advances east by 1/iteration)
  ...
  Per iteration:
    Phase 1 — Read fuel pair, XOR to get zero:
      We need [H0]^=[H1] but H0 is on EX row, not fuel row.
      x does [H0] ^= [H1], putting result in H0's cell (EX row). No good.
  ...
  zero in a CELL, but we need that zero to end up on the EX row, and
  ...
  Now use Z (swap [H0] with [EX]) to swap this zero with the EX cell.
  Z doesn't need H0 on the EX row — it swaps [H0] with [EX] regardless
  of where H0 is!
  ...
  From CLAUDE.md:
    ...
    No wait: Z (38): swap([H0], [EX]) — byte-level EX swap

  Let me re-check the ISA table...
    T (22): swap([CL], [H0]) — bridge
    Z (38): swap([H0], [EX]) — byte-level EX swap

  YES! Z swaps [H0] with [EX]. So:
  - H0 at fuel[2i] (which is 0 after XOR)
  - EX at gp_trail[i]
  - Z: gp_trail[i] gets 0, fuel[2i] gets old gp_trail[i] value

  This is exactly what we need! And we don't need to move H0 to the EX
  row at all. H0 stays on the fuel row.

  After Z: fuel[2i] has the old EX value (breadcrumb or 0).
  gp_trail[i] has 0 (ready for next P).

  Then ] advances EX east (to cell i+1 for next iteration).

  REVISED GADGET:
    e     H1 east → fuel[2i+1]
    x     [H0] ^= [H1] → fuel[2i] = 0
    Z     swap [H0] with [EX] → gp_trail gets 0, fuel gets old gp value
    ]     EX east (advance for next iteration)
    P     [EX]++ (breadcrumb — the "useful work" consuming a zero)
  ...
  - After x: fuel[2i]=0, H0 at (0,2i), EX at (3,i)
  - Z: swap(fuel[2i], gp_trail[i]) → fuel[2i]=old_gp[i], gp_trail[i]=0
  - ]: EX moves east → EX at (3,i+1)
  - P: gp_trail[i+1]++ → writes breadcrumb (0→1) in cell i+1
    ...
    advances EX to i+2, and P writes to i+2.
  ...
  Actually wait. First iteration: EX starts at cell 0.
  ...
  The EX trail has at most one breadcrumb at any time. The fuel row
  accumulates the old breadcrumbs (values of 1) in the even positions.

  This works! The Z+]+P sequence is a rolling cleanup.

  On a torus wrap: when EX wraps around, cell 0 has the old breadcrumb
  ...
  zero to EX because EX's cell was already 0.

  Fix: start EX at cell 0, but do P FIRST (before Z). Then Z cleans up
  ...
  absorbs one breadcrumb. The EX trail stays clean (all zeros after Z).

  But there's a problem: on the FIRST iteration, ( would check [EX] before
...
def make_torus_agent(fuel_pairs):
    """Build a torus-looping fuel agent with EX shuttle.
    ...
    # Grid must be wide enough for fuel and for EX trail.
    # EX trail needs n_pairs + 1 cells (P writes one per iteration plus
    # one final position).
...
def run_test(fuel_pairs, label="", verbose=False):
    """Run a torus agent test with EX shuttle."""
    sim, n_pairs, percent_col, orig_fuel = make_torus_agent(fuel_pairs)
    cols = sim.cols

    print(f"\n{'='*60}")
    print(f"Torus agent + EX shuttle: {fuel_pairs}  {label}")
    print(f"{'='*60}")
    ...
    for step in range(max_steps):
        ...
        if verbose and sim.step_count <= 50:
            ...
            print(f"    step {sim.step_count:3d}: IP=({r},{c})={opch:2s}"
                  f"  H0=({h0_r},{h0_c})  H1=({h1_r},{h1_c})  EX=({ex_r},{ex_c})")
    ...
    # EX trail should be all zeros (each P is cleaned by Z).
    gp_used = gp_after_p[:n_pairs + 1]
    gp_clean = all(v == 0 for v in gp_used)

    print(f"    EX trail clean: {'YES' if gp_clean else 'NO'} {gp_used}")
...
if __name__ == '__main__':
    ...
    if all_ok:
        ...
        print(f"  P     — write breadcrumb (useful work, consumes EX zero)")
        print(f"  Z     — shuttle: swap zero from fuel to EX trail")
        print(f"  ]     — advance EX for next iteration")
